...
namespace MyPG
{
	...
	void MyGameEngine::CreateEffect(int no, ML::Vec2 pos) {
		...
		case 10:// ？
			if (auto e = Effect00::Object::Create(true)) {
				e->filename = "data/effect/warning.png";//800x300
				e->resSizeX = 400;
				e->resSizeY = 300;
				e->drawSizeX = 400;
				e->drawSizeY = 300;
		...
		case 50://プレイヤー退場
			if (auto e = Effect00::Object::Create(true)) {
				e->filename = "data/effect/player.png";//800x300
				e->resSizeX = 32;
				e->resSizeY = 32;
				e->drawSizeX = 64;
				e->drawSizeY = 64;
...
*/
	...
	MyGameEngine::MyGameEngine()
		:GameEngine(
			...
			480,							//	2D描画用解像度X(現在非対応）
			270,							//	2D描画用解像度Y(現在非対応）
			"2023年度　制作実習　シューティングゲーム　「長谷川 勇一朗」",	//	ウインドウタイトル
			"GPG",							//	ウィンドウクラス名
	...
	bool MyGameEngine::Initialize(HWND wnd_)
	{
		...
		//デバッグ用フォントの準備
		this->debugFont = DG::Font::Create("ＭＳ ゴシック", 8, 22);
